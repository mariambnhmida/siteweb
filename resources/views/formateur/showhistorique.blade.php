                      ...
                      <table class="table">
                        <thead>
                          <tr>
                            ...
                            <th class="font-weight-bold">Historique de participation</th>

                          </tr>
                        </thead>
                        <tbody>
                            @foreach ($formations as $formations)
                              @foreach ($formations->achatformations as $l)
                                <tr>
                                @foreach($users as $user)
                                @if ($user->id == $l->user_id)
                                <td><img src="{{asset('/'.$user->profile_photo_path)}}" class="img-fluid"  width="80px" height="80px">
                                    {{$user->name}} </td>
                                @endif
                            ...
                            <td>{{$user->email}} </td>
                              @endif
                          @endforeach
                          <td>{{$l->formations->nom}} </td>
                          <td>{{$l->created_at->translatedFormat('d M Y à H\hi')}} </td>
                          <td>
                            <center>
                                <button type="button" class="btn btn-success btn-sm" >
                                    <a href="{{route('historiqp',$formations->id)}}" style="color:rgb(253, 251, 249)">Consulter

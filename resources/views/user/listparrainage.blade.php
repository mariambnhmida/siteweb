...
<li>
    <a class="getstarted scrollto" href="{{ route('showparrain') }}" >
        <i class="icon-user-follow"></i> Ajouter un parrain</a>
    </li>
    ...
    <section id="services" class="services section-bg">
        <div class="container" data-aos="fade-up">
          <div class="section-title">
            <h2>Liste parrainages</h2>
            <p><a  href="{{ route('showparrain') }}"  class="btn-get-started scrollto">JE PARRAINE</a></p>
          </div>
          <div class="row">
                @php
                $x = 0;
                ...
                <div class="col-md-8 col-lg-3 d-flex align-items-stretch"  data-aos="zoom-in" data-aos-delay="{{$x+=100}}">
                    <div class="icon-box">
                        @if ($parrainage->etat == 'parrinée')
                        @foreach ($users as $user)
                        @if ($user->email == $parrainage->mail )
                        <div class="icon"><img src="{{asset('/image/'.$user->profile_photo_path)}}" class="img-fluid"></div>
                        @endif
                        @endforeach
                        @endif
                        @if ($parrainage->etat == 'en attente' )
                        <div class="icon"><img src="{{asset('site/assets/img/imh.png')}}" class="img-fluid" ></div>
                        @endif
                        <h4 class="title">Prénom de votre filleul(e) :</h4>
                        <p class="description">  {{$parrainage->new_id}} </p>
                        <h4 class="title">Email de votre filleul(e) :</h4>
                        <p class="description">  {{$parrainage->mail}} </p>
                        <h4 class="title">Etat :</h4>
                        @if ($parrainage->etat == 'parrinée')
                        <div class="badge badge-success p-2">{{$parrainage->etat}} <i class="fas fa-add"></i> </div>
                        @endif
                        @if ($parrainage->etat == 'en attente' )
                        <div class="badge badge-warning p-2">{{$parrainage->etat}} <i class="fas fa-add"></i> </div>
                        @endif
                    </div>
                </div>
                @endforeach
